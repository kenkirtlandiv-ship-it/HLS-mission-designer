import streamlit as st
import math

# --- PAGE CONFIG ---
st.set_page_config(page_title="HLS Mission Console", layout="wide")

# --- PURE BLACK & MINT CSS ---
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=JetBrains+Mono&display=swap');

    /* Global Theme */
    .stApp {
        background-color: #000000;
        color: #98FF98;
        font-family: 'JetBrains Mono', monospace;
    }

    /* Mint text for all standard elements */
    h1, h2, h3, p, span, label, div {
        color: #98FF98 !important;
        text-transform: uppercase;
    }

    /* Sliders */
    .stSlider [data-baseweb="slider"] { background-color: transparent; }

    /* Custom Button Grid Logic */
    .stButton > button {
        width: 100%;
        border: 1px solid #98FF98 !important;
        background-color: transparent !important;
        color: #98FF98 !important;
        border-radius: 0px;
        height: 50px;
        font-size: 10px !important;
    }

    .stButton > button:hover {
        background-color: rgba(152, 255, 152, 0.1) !important;
    }

    /* Horizontal line */
    hr { border: 0; border-top: 1px solid #98FF98; margin: 20px 0; }

    /* Tanker visualization container */
    .tanker-container {
        display: flex;
        flex-wrap: wrap;
        gap: 5px;
        justify-content: flex-start;
        margin-top: 10px;
    }
    </style>
    """, unsafe_allow_html=True)

# --- SVG PLACEHOLDERS ---
# (Replace these strings with your SVG code later)
TANKER_SVG = """<svg width="80" height="110" viewBox="0 0 60 100" xmlns="http://www.w3.org/2000/svg"><path d="M30 5 L15 25 L15 85 L10 95 L50 95 L45 85 L45 25 Z" fill="#C0C0C0" stroke="#98FF98"/><path d="M15 25 L10 22 L10 30 M45 25 L50 22 L50 30 M15 80 L5 85 L15 90 M45 80 L55 85 L45 90" stroke="#98FF98"/></svg>"""
HLS_SVG = """<svg width="80" height="110" viewBox="0 0 60 100" xmlns="http://www.w3.org/2000/svg"><path d="M30 5 L15 30 L15 85 L45 85 L45 30 Z" fill="#FFFFFF" stroke="#98FF98"/><rect x="25" y="15" width="10" height="5" fill="#333"/><line x1="15" y1="85" x2="8" y2="98" stroke="#98FF98" stroke-width="2"/><line x1="45" y1="85" x2="52" y2="98" stroke="#98FF98" stroke-width="2"/></svg>"""
ORION_SVG = """<svg width="100" height="100" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"><path d="M40 45 L50 25 L60 45 Z" fill="#C0C0C0" stroke="#98FF98"/><rect x="42" y="45" width="16" height="15" fill="#C0C0C0" stroke="#98FF98"/><rect x="5" y="50" width="35" height="4" fill="#555" stroke="#98FF98"/><rect x="60" y="50" width="35" height="4" fill="#555" stroke="#98FF98"/></svg>"""

# --- SESSION STATE FOR BUTTONS ---
if 'push_mode' not in st.session_state:
    st.session_state.push_mode = "MODE 1: NO PUSH"

def set_mode(mode):
    st.session_state.push_mode = mode

# --- UI INPUTS ---
st.markdown("<h2 style='text-align: center;'>HLS LOGISTICS TERMINAL</h2>", unsafe_allow_html=True)
st.markdown("---")

col1, col2, col3 = st.columns(3)

with col1:
    st.markdown(TANKER_SVG, unsafe_allow_html=True)
    refill_amount = st.slider("PROP PER REFILL (t)", 50, 200, 100, step=5)
    cadence = st.slider("LAUNCH CADENCE (DAYS)", 1, 31, 7)

with col2:
    st.markdown(HLS_SVG, unsafe_allow_html=True)
    dry_mass = st.slider("HLS DRY MASS (t)", 80, 250, 120, step=5)
    isp = st.slider("ENGINE ISP (s)", 350, 380, 375)
    orbit = st.selectbox("STAGING ORBIT", ["NRHO", "LLO", "PCO"])

with col3:
    st.markdown(ORION_SVG, unsafe_allow_html=True)
    st.write("ORION MISSION MODES")
    
    # 2x2 Button Grid
    btn_col_a, btn_col_b = st.columns(2)
    
    m1 = "MODE 1: NO PUSH"
    m2 = "MODE 2: LEO TO STAGING"
    
    # Logic for specialized transfer modes
    transfer_modes = []
    if orbit == "LLO":
        transfer_modes = ["MODE 3: NRHO TO LLO", "MODE 4: PCO TO LLO"]
    elif orbit == "PCO":
        transfer_modes = ["MODE 3: NRHO TO PCO", "N/A"]
    else:
        transfer_modes = ["N/A", "N/A"]

    with btn_col_a:
        if st.button(m1, on_click=set_mode, args=(m1,)): pass
        if st.button(transfer_modes[0], on_click=set_mode, args=(transfer_modes[0],)) and transfer_modes[0] != "N/A": pass
        
    with btn_col_b:
        if st.button(m2, on_click=set_mode, args=(m2,)): pass
        if st.button(transfer_modes[1], on_click=set_mode, args=(transfer_modes[1],)) and transfer_modes[1] != "N/A": pass

    # Highlight active mode
    st.markdown(f"SELECTED: <span style='background-color:#98FF98; color:black !important; padding: 2px 5px;'>{st.session_state.push_mode}</span>", unsafe_allow_html=True)

# --- MATH CORE ---
G = 9.80665
ORION_M = 27.0
DV = {
    "LEO_TO_TLI": 3200, "TLI_TO_NRHO": 450, "TLI_TO_LLO": 900, "TLI_TO_PCO": 800,
    "NRHO_TO_SURFACE": 2750, "LLO_TO_SURFACE": 2000, "PCO_TO_SURFACE": 2400
}

def calculate():
    mode = st.session_state.push_mode
    
    # 1. Ascent (Ends at Dry Mass)
    dv_ascent = DV[f"{orbit}_TO_SURFACE"]
    m_final_ascent = dry_mass
    m_init_ascent = m_final_ascent * math.exp(dv_ascent / (isp * G))
    
    # 2. Descent (Ends at m_init_ascent)
    dv_descent = dv_ascent
    m_final_descent = m_init_ascent
    m_init_descent = m_final_descent * math.exp(dv_descent / (isp * G))
    
    m_current = m_init_descent

    # 3. Intermediate Orion Pushes
    if "MODE 3" in mode or "MODE 4" in mode:
        dv_push = 0
        if "NRHO TO LLO" in mode: dv_push = 450
        elif "PCO TO LLO" in mode: dv_push = 400
        elif "NRHO TO PCO" in mode: dv_push = 350
        
        m_final_push = m_current + ORION_M
        m_init_push = m_final_push * math.exp(dv_push / (isp * G))
        m_current = m_init_push - ORION_M

    # 4. TLI Arrival
    dv_tli = 3200 + DV[f"TLI_TO_{orbit}"]
    payload_on_tli = ORION_M if "MODE 2" in mode else 0
    m_final_tli = m_current + payload_on_tli
    m_init_tli = m_final_tli * math.exp(dv_tli / (isp * G))
    
    total_prop = m_init_tli - dry_mass - payload_on_tli
    if "MODE 3" in mode or "MODE 4" in mode:
        total_prop = m_init_tli - dry_mass
        
    num_tankers = math.ceil(total_prop / refill_amount)
    return total_prop, num_tankers, m_init_tli

total_prop, num_tankers, leo_mass = calculate()

# --- RESULTS ---
st.markdown("---")
r_col1, r_col2, r_col3 = st.columns(3)
with r_col1: st.metric("TANKERS REQUIRED", num_tankers)
with r_col2: st.metric("TOTAL PROPELLANT", f"{total_prop:,.1f} T")
with r_col3: st.metric("CAMPAIGN DURATION", f"{num_tankers * cadence} DAYS")

# Tanker Row
tanker_html = "".join([f"<div style='display:inline-block;'>{TANKER_SVG}</div>" for _ in range(num_tankers)])
st.markdown(f"<div class='tanker-container'>{tanker_html}</div>", unsafe_allow_html=True)

# Math Readout
st.markdown("---")
with st.expander("MISSION TELEMETRY READOUT"):
    st.text(f"""
    STAGING: {orbit} | MODE: {st.session_state.push_mode}
    [1] LEO START: {leo_mass:.1f}t (Wet Mass)
    [2] ARRIVAL @ {orbit}: {(leo_mass * math.exp(-(3200 + DV[f'TLI_TO_{orbit}']) / (isp * G))):.1f}t
    [3] LUNAR SURFACE: {(dry_mass * math.exp(DV[f'{orbit}_TO_SURFACE'] / (isp * G))):.1f}t
    [4] RETURN @ {orbit}: {dry_mass:.1f}t (Dry Mass)
    """)
